...
def test_lookup_is_cached_and_repeated_calls_agree() -> None:
    # A private per-instance cache, keyed on the raw model string: a session with
    # thousands of spans sharing a handful of distinct models must not re-run the linear
    # prefix scan for every span. Verified directly against the cache dict rather than by
    # intercepting str.startswith, which CPython does not allow monkeypatching.
    table = RateTable.load()
    first = table.lookup("claude-fable-5-1")
    assert table._lookup_cache == {"claude-fable-5-1": first}  # pylint: disable=protected-access
    for _ in range(50):
        assert table.lookup("claude-fable-5-1") == first
    assert table._lookup_cache == {"claude-fable-5-1": first}  # pylint: disable=protected-access


def test_lookup_cache_does_not_confuse_different_models() -> None:
    table = RateTable.load()
    a = table.lookup("claude-fable-5-1")
    b = table.lookup("claude-opus-5")
    assert a is not None and b is not None
    assert a.match != b.match
    assert table.lookup("claude-fable-5-1") == a
    assert table.lookup("claude-opus-5") == b


def test_lookup_cache_stores_a_genuine_miss_as_none_not_as_uncached() -> None:
    table = RateTable.load()
    assert table.lookup("no-such-model-anywhere") is None
    assert table.lookup("no-such-model-anywhere") is None
