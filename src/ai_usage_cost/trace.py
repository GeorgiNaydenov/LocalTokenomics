...
class ContextSnapshot(BaseModel):
    ...
    # Category -> estimated share of input_total (0..1, summing to 1.0). Always includes
    # an "Other" entry: content this reader cannot see at all (the system prompt is never
    # logged by any source here) plus whatever the byte-to-token estimate underweighted.
    # Provenance is always "estimated" -- see `_composition_of`'s docstring for the method.
...
_COMPACTION_CATEGORY = "Compaction summary"
_OTHER_CATEGORY = "Other (unlogged or unclassified)"
# A rough, widely-cited English-text heuristic, used only to turn observed record bytes
# into a token estimate comparable against the measured `input_total` -- never used to
# compute a billed figure. Documented rather than tuned, because the true chars-per-token
# ratio varies by content and model; the "Other" bucket absorbs the resulting slack.
_CHARS_PER_TOKEN_ESTIMATE = 4
_COMPOSITION_KINDS: tuple[SpanKind, ...] = (
    "user",
    "assistant",
    "reasoning",
    "tool_call",
...
def _composition_category(span: Span) -> str:
    if span.kind == "user":
    ...
    if span.kind == "reasoning":
        return "Reasoning"
    if span.kind in ("tool_call", "tool_result", "retrieval", "subagent"):
        return f"Tool: {span.name}" if span.name else "Tool calls"
    return _OTHER_CATEGORY


def _composition_of(bucket: dict[str, int], input_total: int) -> dict[str, float]:
    """Estimate what filled the context window at one model call, as a share of the
    *measured* `input_total`, never of the observed bytes alone.

    No source this reader parses logs the real system prompt, and some record kinds are
    not yet classified into their own category either -- both land in `_OTHER_CATEGORY`
    alongside whatever the byte estimate simply underweighted, so "Other" is never hidden
    and never claims more precision than a byte count can support.
    """
    observed_bytes = sum(bucket.values())
    if input_total <= 0:
        return {}
    if observed_bytes <= 0:
        return {_OTHER_CATEGORY: 1.0}
    estimated_tokens = observed_bytes / _CHARS_PER_TOKEN_ESTIMATE
    categorized_fraction = min(1.0, estimated_tokens / input_total)
    composition = {
        category: (count / observed_bytes) * categorized_fraction
        for category, count in bucket.items()
    }
    composition[_OTHER_CATEGORY] = max(0.0, 1.0 - sum(composition.values()))
    return composition
...
def context_of(spans: Sequence[Span], table: RateTable) -> list[ContextSnapshot]:
    ...
    category_bytes: dict[tuple[str, str | None], dict[str, int]] = {}
    for span in spans:
        key = (span.session_id, span.agent_id)
        if span.kind == "compaction":
            # A compaction folds everything before it into one new summary -- what the
            # model carries forward from here is the summary itself, not the record bytes
            # that produced it.
            category_bytes[key] = {_COMPACTION_CATEGORY: span.record_length}
        elif span.kind in _COMPOSITION_KINDS:
            bucket = category_bytes.setdefault(key, {})
            category = _composition_category(span)
            bucket[category] = bucket.get(category, 0) + span.record_length
        ...
        snapshots.append(
            ContextSnapshot(
                ...
                composition=_composition_of(category_bytes.get(key, {}), span.tokens.input_total),
            )
        )
