...
def create_app(
    ...
    # A large session's spans are reconstructed (one SQL fetch, one Pydantic validation per
    # row) from scratch on every request that needs them -- and the trace, economics and
    # context tabs each ask independently, so opening all three for one session paid that
    # cost three times over. Cached per (source, session_id) for the life of this analysis
    # generation; `/api/refresh` below is the only place that generation ever changes, so
    # clearing the cache there is the entire invalidation story.
    span_cache: dict[tuple[str, str], list[Span]] = {}
    ...
    def session_spans(
        ...
        with opened() as conn:
            ...
            if cached is None:
                cached = store.load_spans(conn, source, session_id)
                span_cache[key] = cached
            yield conn, cached
    ...
    def forget_session(source: str, session_id: str) -> dict[str, int]:
        with opened() as conn:
            if not store.session_exists(conn, source, session_id):
                raise HTTPException(404, f"no session {session_id!r} under source {source!r}")
            deleted = store.delete_session(conn, source, session_id)
            span_cache.pop((source, session_id), None)
