    ...
    <div className="flex flex-col gap-3">
      <StatGrid className="xl:grid-cols-2">
        <Stat
          ...
          exact={hasTokens(totals) ? formatExact(totals.tokens.total) : undefined}
          info={
            hasTokens(totals)
              ? {
                  kind: 'metric',
                  meaning: 'Every token this session logged, summed across every model call: input read plus output produced.',
                  formula: 'tokens = input_total + output',
                  computed: `${formatExact(totals.tokens.input_total)} input + ${formatExact(totals.tokens.output)} output = ${formatExact(totals.tokens.total)}`,
                }
              : undefined
          }
        />
        <Stat
          ...
          exact={totals.cost ? formatMoney(totals.cost.total) : undefined}
          info={
            totals.cost
              ? {
                  kind: 'metric',
                  meaning: 'What this session would cost at published API list prices, priced per model call and summed.',
                  formula: 'cost = Σ (uncached input + cache read + cache write + output) × each call’s rate',
                  computed: `uncached ${formatMoney(totals.cost.uncached_input)} + cache read ${formatMoney(totals.cost.cache_read)} + cache write ${formatMoney(totals.cost.cache_write)} + output ${formatMoney(totals.cost.output)} = ${formatMoney(totals.cost.total)}`,
                  caveats:
                    totals.cost.inherited
                      ? 'At least one call in this session used a rate inherited from a sibling model because no exact rate entry exists for it.'
                      : totals.cost_state !== 'priced'
                        ? `State is ${totals.cost_state}: the worst state across this session's calls, so this total may not include every call.`
                        : undefined,
                }
              : undefined
          }
        ...
        <Stat
          ...
          info={{
            kind: 'metric',
            meaning: 'Active time across this session’s turns: an interval union, so time inside a nested subagent turn is never added twice.',
            formula: 'duration = length of the merged union of every turn’s own (start, end) interval',
            caveats: 'This is active time, not wall clock. A session with long idle gaps between turns can show far less duration than the time between its first and last record.',
          }}
        />
        <Stat
          label="Calls"
          ...
          info={{
            kind: 'simple',
            text: 'Every model call plus every tool call logged in this session. A tool call and its result are one call, not two, and a failed call is counted once in "errored" even though the call and its result both carry the error status.',
          }}
        />
        <Stat
          label="Retries"
          value={<RetriesCell row={totals} />}
          hint={retryFoot(props.source)}
          info={{
            kind: 'metric',
            meaning: 'How many extra attempts this session’s requests needed, combining two different signals the log carries.',
            formula: 'retries = max(attempt index seen for a retried request) + Σ(iteration counts already reported as a count, not an index)',
            caveats: 'Three attempt-index records 1, 2, 3 for the same request count as 3 retries, not 6 — summing raw indices would double count.',
          }}
        />
        <Stat
          label="Amplification"
          ...
          info={{
            kind: 'metric',
            meaning: 'How many input tokens were read for every output token produced, including cheap cache reads.',
            formula: 'amplification = input_total ÷ output',
            caveats: 'A large number here is normal and expected on a well-cached agent turn, since cache reads bill far below full price. The warning this can trigger looks only at full-price tokens (uncached input plus cache writes), not this displayed ratio.',
          }}
        />
        <Stat
          ...
          info={{
            kind: 'metric',
            meaning: 'Share of all input tokens that were served from the prompt cache rather than read fresh.',
            formula: 'cache hit ratio = cache read ÷ input_total',
          }}
        />
        <Stat
          ...
          info={{
            kind: 'simple',
            text: "Output tokens divided by duration, computed only when both are measured (not derived or estimated) for the same call. Most clients here log one but not the other on a single call, so this is often unavailable — that is a logging gap, never shown as zero.",
          }}
        />
      </StatGrid>
